# 🏗️ 아키텍처 및 믹스인(Mixins) 분석 (Minecraft 26.3)
...
## 3. 무할당(Zero-Allocation) 고속 경로
* **태그 매칭**: `ItemStack#is(TagKey)`는 $O(1)$ 상수 시간에 실행됩니다.
* **규칙 조회**: `CombatRules.getInt`는 메모리 캐시를 직접 읽어 객체 할당이 없습니다.
* **GC 부하 제로**: `CCAHooks.getCooldownTicks`는 힙 할당을 전혀 생성하지 않아 대규모 전투에서도 렉이 없습니다.
...
* [[26.3 전투 타격감 및 오디오 피드백|ko_kr-26.3-Combat-Juice-and-Audio-Feedback]]
