...
| **Coleção do Mod** | Multi-Version Anchor Subprojects (`26.2` & `26.3`) |
| **Bytecode Injector** | SpongePowered Mixin 0.8+ |
| **Dependência Principal** | Fabric API, DasikLibrary (`DynamicGameRuleManager`) |
...
## 5. Arquitetura Mixin e Padrão de Pureza
O Combat Cooldown Adjuster segue rigorosamente o **Padrão de Pureza Mixin**:
1. **Zero Lógica nos Mixins**: `PlayerMixin.java` não faz cálculos complexos.
2. **Delegação Estática**: Toda a lógica e efeitos residem em `CCAHooks.java`.
3. **Segurança no Cancelamento**: Pontos de injeção em `@At("HEAD")` explícitos com `cancellable = true`.
...
## 6. Ganchos de Extensão para Desenvolvedores de Addons
Integração de Lanças via `#c:spears`:
Basta adicionar seus itens em `data/c/tags/item/spears.json` para que usem automaticamente as regras de lanças.
...
## 7. Links de Documentação Relacionados
